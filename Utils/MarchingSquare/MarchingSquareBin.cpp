...
void MarchingSquareBin::contourSimplification(double epsilon)
{
    QMeshPatch *m_contour_simplified = new QMeshPatch;
    const int numIterations(20);
    int localCount = 0;
    for (auto iter = m_contour->FaceBegin(); iter != m_contour->FaceEnd(); iter = iter->Next()) {
        auto face = m_contour->GetFaceAt(iter); // each face store an edge loop
        list<Eigen::Vector3d> points;
        Eigen::Vector3d p;
        for(int i = 1; i < face->GetEdgeNum(); i++) {
            auto t = face->GetNodeRecordPtr(i);
            t->GetCoord3D(p[0],p[1],p[2]);
            points.emplace_back(p);
        }

        // reduce points if length between consectuitive points is less than epsilon
        for (auto ptr1(points.begin()); ptr1 != points.end();) {
            auto ptr2 = ptr1; ++ptr2;
            if (ptr2 == points.end()) break;

            // calculate point-point distance
            while ((*ptr2 - *ptr1).norm() <= epsilon && ptr2 != points.end()) {
                ptr2 = points.erase(ptr2);
            }
            ptr1 = ptr2;
        }
        if (points.size() <= 2) continue;

//        // reduce points if chord length is less than epsilon
//        for (int i = 0; i < numIterations; ++i) {
//            int numBefore(points.size());
//            for (auto ptr1(points.begin()); ptr1 != points.end();) {
//                auto ptr2 = ptr1; ++ptr2;
//                if (ptr2 == points.end()) break;
//                auto ptr3 = ptr2; ++ptr3;
//                if (ptr3 == points.end()) break;

//                // calculate point to segment distance
//                auto dir = (*ptr3 - *ptr1).normalized();
//                auto vec = *ptr2 - *ptr1;
//                auto dist = (vec - vec.dot(dir) * dir).norm();
//                if (dist <= epsilon)ptr1 = points.erase(ptr2);
//                else ptr1 = ptr3;
//            }
//            if (points.size() == numBefore) break;
//        }
//        if (points.size() <= 2) continue;

        // put points into the edges
        QMeshFace* curFace;
        QMeshEdge* newedge;
        QMeshNode* nextNode;
        QMeshNode* prevNode;

        curFace = new QMeshFace();
        curFace->SetMeshPatchPtr(m_contour_simplified);
        curFace->SetIndexNo(m_contour_simplified->GetFaceNumber());
        m_contour_simplified->GetFaceList().AddTail(curFace);
        prevNode = new QMeshNode();
        auto firstNode = prevNode;
        prevNode->SetCoord3D(points.front()[0], points.front()[1], points.front()[2]);
        prevNode->AddFace(curFace);
        int edgeCount = 0;
        for (auto iter(++points.begin()); iter != points.end(); ++iter) {
            nextNode = new QMeshNode();
            nextNode->SetCoord3D((*iter)[0], (*iter)[1], (*iter)[2]);
            nextNode->AddFace(curFace);
            newedge = new QMeshEdge();
            newedge->SetLeftFace(curFace);
            newedge->SetStartPoint(prevNode);
            prevNode->AddEdge(newedge);
            m_contour_simplified->GetEdgeList().AddTail(newedge);
            m_contour_simplified->GetNodeList().AddTail(prevNode);
            prevNode->SetIndexNo(localCount);
            newedge->SetIndexNo(localCount);
            curFace->SetEdgeRecordPtr(edgeCount, newedge);
            curFace->SetDirectionFlag(edgeCount, true);
            newedge->SetEndPoint(nextNode);
    ...
    delete m_contour;
    m_contour = m_contour_simplified;
